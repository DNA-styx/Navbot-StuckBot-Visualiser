...
public void OnMapStart()
{
    ...
    PrintToServer("[StuckBots] Map started, loading stuck bot locations...");

    char map[64];
    GetCurrentMap(map, sizeof(map));

    char path[PLATFORM_MAX_PATH];
    BuildPath(Path_SM, path, sizeof(path), "data/stuck-bots/stuck-bots.txt");

    if (!FileExists(path))
    {
        PrintToServer("[StuckBots] File not found: %s", path);
        return;
    }
    ...
    {
        PrintToServer("[StuckBots] No stuck bot entries found.");
        CloseHandle(kv);
        return;
    }
    ...
    do
    {
        ...
        {
            PrintToServer("[StuckBots] Maximum stuck bot limit reached.");
            break;
        }
    ...
    PrintToServer("[StuckBots] Stuck bots for map %s:", map);
    for (int i = 0; i < g_StuckCount; i++)
    {
        PrintToServer("  Location %d: %f %f %f", i+1, g_StuckX[i], g_StuckY[i], g_StuckZ[i]);
    }
    PrintToServer("[StuckBots] Total stuck bots for map %s: %d", map, g_StuckCount);
